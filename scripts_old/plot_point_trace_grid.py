...
Usage example:
    python plot_point_trace_grid.py --sample_index 5 --point_index 0
    python plot_point_trace_grid.py --sample_index 5 --point_index 0 --dominant_search_t0 20 --dominant_search_t1 60  
    python plot_point_trace_grid.py --sample_index 5 --point_index 0 --zoom_half_width_sec 8
    python plot_point_trace_grid.py --sample_index 5 --point_index 2 --zoom_half_width_sec 8
    python plot_point_trace_grid.py --sample_index 5 --point_index 2 --zoom_half_width_sec 8
"""
...
def plot_one_point_local_crest_column(
    ...
    for row, item in enumerate(loaded):
        ...
        yrange = ymax - ymin
        y_arrow_t = ymax - 0.14 * yrange

        x0 = min(t_true, t_pred)
        x1 = max(t_true, t_pred)

        if abs(x1 - x0) < 1e-5:
            x0 = t_true - 0.025 * (zoom_t1 - zoom_t0)
            x1 = t_true + 0.025 * (zoom_t1 - zoom_t0)
        ...
        x_arrow_eta = min(
            max(t_pred + 0.06 * (zoom_t1 - zoom_t0), zoom_t0 + 0.06 * (zoom_t1 - zoom_t0)),
            zoom_t1 - 0.07 * (zoom_t1 - zoom_t0),
        )
        ...
        ax.text(
            x_arrow_eta + 0.020 * (zoom_t1 - zoom_t0),
            0.5 * (y0 + y1),
    ...
    out_png = out_dir / (
        f"sample{sample_index + 1:02d}_point{point_index + 1:02d}_local_crest_{suffix}.png"
    )
    fig.savefig(out_png)
    plt.close(fig)
