...
class SimpleCNNRefiner(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, base_channels: int = 64, depth: int = 4, dropout: float = 0.0):
        ...
        layers: List[nn.Module] = [nn.Conv2d(in_channels, base_channels, 3, padding=1), nn.GELU()]
        for _ in range(depth - 1):
            layers += [nn.Conv2d(base_channels, base_channels, 3, padding=1), nn.GELU()]
            if dropout > 0:
...
def restore_config_from_checkpoint(config: SeaSurfaceRolloutConfig, checkpoint: Dict[str, Any]) -> SeaSurfaceRolloutConfig:
    ...
    if isinstance(ckpt_cfg, dict):
        for k, v in ckpt_cfg.items():
            if hasattr(config, k):
                if k in {"n_modes", "rollout_train_steps", "rollout_curriculum_boundaries", "plot_future_steps", "spectral_band_split_ratios"} and isinstance(v, list):
                    v = tuple(v)
                setattr(config, k, v)
    return config
...
@torch.no_grad()
def c2f_predict(model: CoarseToFineModel, x_init: torch.Tensor, rollout_steps: int, config: SeaSurfaceRolloutConfig) -> Dict[str, torch.Tensor]:
    model.eval()
    use_refiner = str(getattr(config, "c2f_train_stage", "joint")).lower() != "coarse_only"
    return model(x_init, target_steps=int(rollout_steps), use_refiner=use_refiner, detach_context=False)
...
@torch.no_grad()
def evaluate_rollout(model: CoarseToFineModel, loader: DataLoader, device: torch.device, config: SeaSurfaceRolloutConfig, data_mean: float, data_std: float, split: str) -> Dict[str, Any]:
    model.eval()
    ...
    total_rel_l2 = 0.0
    total_samples = 0
    base_total_sqerr = 0.0
    base_total_count = 0

    step_sqerr = np.zeros(rollout_steps, dtype=np.float64)
    step_count = np.zeros(rollout_steps, dtype=np.int64)
    base_step_sqerr = np.zeros(rollout_steps, dtype=np.float64)
    base_step_count = np.zeros(rollout_steps, dtype=np.int64)
    ...
    for batch in loader:
        ...
        out = c2f_predict(model, x, rollout_steps, config)
        sync_if_cuda(device)
        ...
        sq_err = (pred - y) ** 2
        base_sq_err = (base - y) ** 2
        total_sqerr += sq_err.sum().item()
        total_count += sq_err.numel()
        base_total_sqerr += base_sq_err.sum().item()
        base_total_count += base_sq_err.numel()
        ...
        for t in range(rollout_steps):
            ...
            base_step_sqerr[t] += base_sq_err[:, t].sum().item()
            base_step_count[t] += base_sq_err[:, t].numel()
        ...
        for local_idx in range(bsz):
            ...
            saved_y_base.append(base[local_idx:local_idx + 1].cpu().numpy().astype(np.float32))
    ...
    coarse_base_rmse = float(np.sqrt(base_total_sqerr / max(base_total_count, 1)))
    rel_l2 = float(total_rel_l2 / max(total_samples, 1))
    stepwise_rmse = np.sqrt(step_sqerr / np.maximum(step_count, 1)).astype(np.float32)
    base_stepwise_rmse = np.sqrt(base_step_sqerr / np.maximum(base_step_count, 1)).astype(np.float32)
    ...
    rollout_infer_time_per_batch_sec = float(np.mean(rollout_infer_time_batch_arr)) if rollout_infer_time_batch_arr.size > 0 else float("nan")
    rollout_infer_time_per_sample_sec = float(rollout_infer_time_total_sec / rollout_infer_num_samples) if rollout_infer_num_samples > 0 else float("nan")
    ...
    result = {
        ...
        "coarse_base_rmse": np.array(coarse_base_rmse, dtype=np.float32),
        ...
        "base_stepwise_rmse": base_stepwise_rmse,
        ...
        "y_base_plot": np.concatenate(saved_y_base, axis=0) if saved_y_base else np.empty((0, rollout_steps, 0, 0), dtype=np.float32),
        ...
        "use_coarse_to_fine": np.array(1, dtype=np.int32),
        "c2f_train_stage": np.asarray(str(getattr(config, "c2f_train_stage", "joint")), dtype=object),
        "c2f_refiner_type": np.asarray(str(getattr(config, "c2f_refiner_type", "unet")), dtype=object),
        "c2f_coarse_time_factor": np.array(int(getattr(config, "c2f_coarse_time_factor", 4)), dtype=np.int32),
...
def main(split: str = "val") -> None:
    ...
    load_model_state_dict_safely(model, checkpoint["model_state_dict"], strict=True)
    ...
    summary_row = {
        ...
        "coarse_base_rmse": float(result["coarse_base_rmse"]),
        ...
        "use_coarse_to_fine": 1,
        "c2f_train_stage": str(getattr(config, "c2f_train_stage", "joint")),
        "c2f_refiner_type": str(getattr(config, "c2f_refiner_type", "unet")),
    ...
    print(
        f"C2F rollout inference time ({split}) | "
    ...
    print(
        f"Metrics ({split}) | rmse={summary_row['rmse']:.6f} | "
        f"coarse_base_rmse={summary_row['coarse_base_rmse']:.6f} | "
        f"final_step_rmse={summary_row['final_step_rmse']:.6f} | "
        f"last_chunk_rmse={summary_row['last_chunk_rmse']:.6f}"
    )
...
if __name__ == "__main__":
    split_arg = sys.argv[1].lower() if len(sys.argv) > 1 else "val"
    if split_arg not in {"val", "test"}:
        raise ValueError(f"Unsupported split: {split_arg}. Expected 'val' or 'test'.")
    main(split_arg)
