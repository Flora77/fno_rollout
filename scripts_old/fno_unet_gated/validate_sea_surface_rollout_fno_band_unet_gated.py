...
from config.sea_surface_rollout_config_fno_band_unet_gated import SeaSurfaceRolloutConfig

try:
    from neuralop.models.fno import FNO, TFNO
    from neuralop.models.fno_band_unet_gated_decoder import FNOBandUNetGatedDecoder
...
def build_model(config: SeaSurfaceRolloutConfig) -> torch.nn.Module:
    ...
    if arch in {"fno_band_unet_gated_decoder", "fno_unet_band_gated_decoder", "fno_band_unet_decoder"}:
        return FNOBandUNetGatedDecoder(
            ...
            band_branch_base_channels=int(getattr(config, "fno_unet_band_branch_base_channels", 16)),
            band_cutoffs=tuple(getattr(config, "fno_unet_band_cutoffs", (0.33, 0.67))),
            band_transition_width=float(getattr(config, "fno_unet_band_transition_width", 0.04)),
            band_fusion=str(getattr(config, "fno_unet_band_fusion", "learnable_gate")),
            band_gate_hidden_channels=int(getattr(config, "fno_unet_band_gate_hidden_channels", 32)),
            padding_mode=str(getattr(config, "fno_unet_padding_mode", "periodic")),
        )

    if arch == "fno_unet_decoder":
        raise ValueError(
            "This script is for the new frequency-band gated model. "
            "Set model_arch='fno_band_unet_gated_decoder', or use the old "
            "train/validate scripts for model_arch='fno_unet_decoder'."
        )
