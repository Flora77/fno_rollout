...
def evaluate_rollout(model: torch.nn.Module, loader: DataLoader, device: torch.device, config: SeaSurfaceRolloutConfig, data_mean: float, data_std: float, split: str) -> Dict[str, Any]:
    ...
    # Paper metrics accumulated over the whole validation/test set.
    # They are computed on denormalized physical sea-surface elevations and
    # averaged over samples, prediction time, and spatial grids together:
    #   NRMSE = RMSE / Hs, Hs = 4 * std(eta_true)
    #   SSP   = ||F_xy(pred) - F_xy(true)||_2 / (||F_xy(pred)||_2 + ||F_xy(true)||_2)
    #   Corr  = Pearson correlation over all predicted spacetime grid values.
    metric_count = 0
    metric_sqerr_sum = 0.0
    metric_true_sum = 0.0
    metric_pred_sum = 0.0
    metric_true_sq_sum = 0.0
    metric_pred_sq_sum = 0.0
    metric_cross_sum = 0.0
    metric_fft_diff_sq_sum = 0.0
    metric_fft_true_sq_sum = 0.0
    metric_fft_pred_sq_sum = 0.0
    ...
    for batch in loader:
        ...
        # Accumulate NRMSE, SSP, and Corr on physical-valued fields.
        y_metric = y_phy.astype(np.float64, copy=False)
        pred_metric = pred_phy.astype(np.float64, copy=False)
        diff_metric = pred_metric - y_metric
        metric_sqerr_sum += float(np.sum(diff_metric ** 2))
        metric_count += int(y_metric.size)
        metric_true_sum += float(np.sum(y_metric))
        metric_pred_sum += float(np.sum(pred_metric))
        metric_true_sq_sum += float(np.sum(y_metric ** 2))
        metric_pred_sq_sum += float(np.sum(pred_metric ** 2))
        metric_cross_sum += float(np.sum(y_metric * pred_metric))

        fft_true = np.fft.rfft2(y_metric, axes=(-2, -1), norm="ortho")
        fft_pred = np.fft.rfft2(pred_metric, axes=(-2, -1), norm="ortho")
        metric_fft_diff_sq_sum += float(np.sum(np.abs(fft_pred - fft_true) ** 2))
        metric_fft_true_sq_sum += float(np.sum(np.abs(fft_true) ** 2))
        metric_fft_pred_sq_sum += float(np.sum(np.abs(fft_pred) ** 2))
    ...
    eps_metric = 1e-12
    metric_count_safe = max(int(metric_count), 1)
    rmse_phys = float(math.sqrt(metric_sqerr_sum / metric_count_safe))
    true_mean_metric = metric_true_sum / metric_count_safe
    true_var_metric = max(metric_true_sq_sum / metric_count_safe - true_mean_metric ** 2, 0.0)
    hs_ref = float(4.0 * math.sqrt(max(true_var_metric, eps_metric)))
    nrmse = float(rmse_phys / max(hs_ref, eps_metric))
    ssp = float(
        math.sqrt(max(metric_fft_diff_sq_sum, 0.0))
        / (math.sqrt(max(metric_fft_true_sq_sum, 0.0)) + math.sqrt(max(metric_fft_pred_sq_sum, 0.0)) + eps_metric)
    )
    pred_mean_metric = metric_pred_sum / metric_count_safe
    cov_metric = metric_cross_sum - metric_true_sum * metric_pred_sum / metric_count_safe
    true_ss_metric = metric_true_sq_sum - metric_true_sum ** 2 / metric_count_safe
    pred_ss_metric = metric_pred_sq_sum - metric_pred_sum ** 2 / metric_count_safe
    corr = float(cov_metric / (math.sqrt(max(true_ss_metric, eps_metric)) * math.sqrt(max(pred_ss_metric, eps_metric))))
...
def main(split: str = "val", experiment_name: str = None, checkpoint_dir: str = None, checkpoint_path: str = None) -> None:
    ...
    print(
        f"Paper metrics ({split}) | "
        f"NRMSE={float(result['nrmse']):.6f} | "
        f"SSP={float(result['ssp']):.6f} | "
        f"Corr={float(result['corr']):.6f} | "
        f"RMSE_phys={float(result['rmse_phys']):.6f} | "
        f"Hs_ref={float(result['hs_ref']):.6f}"
    )
    result.update({
