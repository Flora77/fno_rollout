...
try:
    from config.sea_surface_rollout_config_fno_cnn import SeaSurfaceRolloutConfig
except ImportError:
...
class CNNResidualRefiner(nn.Module):
    """
    Local CNN refiner for FNO outputs.

    Input shape:
        x_context: [B, input_steps, H, W]
        coarse:    [B, output_steps, H, W]

    The CNN treats time frames as channels and learns a local spatial correction.
    With residual mode enabled:
        output = coarse + residual_scale * CNN([coarse, x_context])
    """
    def __init__(
        self,
        input_steps: int,
        ...
        in_channels = self.output_steps + (self.input_steps if self.use_context else 0)
        ...
        # Start from the original FNO prediction. This makes the first epoch behave
        # close to the baseline and lets the CNN learn only the missing correction.
        if bool(zero_init_last):
            nn.init.zeros_(self.out_conv.weight)
            if self.out_conv.bias is not None:
                nn.init.zeros_(self.out_conv.bias)

    def forward(self, coarse: torch.Tensor, x_context: torch.Tensor) -> torch.Tensor:
        if self.use_context:
            z = torch.cat([coarse, x_context], dim=1)
        else:
            z = coarse

        correction = self.out_conv(self.body(z))
        if self.use_residual:
            return coarse + self.residual_scale * correction
        return correction


class FNOCNNModel(nn.Module):
    """
    FNO global propagator + CNN local residual refiner.

    FNO captures low-/mid-frequency long-range wave propagation in spectral space.
    CNN then repairs local high-frequency detail, crest/trough sharpness, and
    short-range phase errors in physical space.
    """
    def __init__(self, backbone: nn.Module, config: "SeaSurfaceRolloutConfig") -> None:
        super().__init__()
        self.backbone = backbone
        self.refiner = CNNResidualRefiner(
            input_steps=int(config.input_steps),
            output_steps=int(config.output_steps),
            base_channels=int(getattr(config, "fno_cnn_base_channels", 32)),
            depth=int(getattr(config, "fno_cnn_depth", 4)),
            kernel_size=int(getattr(config, "fno_cnn_kernel_size", 3)),
            dilation=int(getattr(config, "fno_cnn_dilation", 1)),
            dropout=float(getattr(config, "fno_cnn_dropout", 0.0)),
            use_context=bool(getattr(config, "fno_cnn_use_context", True)),
            use_residual=bool(getattr(config, "fno_cnn_use_residual", True)),
            residual_scale=float(getattr(config, "fno_cnn_residual_scale", 0.2)),
            zero_init_last=bool(getattr(config, "fno_cnn_zero_init_last", True)),
        )

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        coarse = self.backbone(x)
        return self.refiner(coarse, x)
...
def build_model(config: SeaSurfaceRolloutConfig) -> nn.Module:
    ...
    if arch in {"fno", "tfno"}:
        model_cls = FNO if arch == "fno" else TFNO
        kwargs = dict(
    ...
    if arch in {"fno_cnn", "tfno_cnn"}:
        # The backbone can be forced by model_arch or overridden by fno_cnn_backbone_arch.
        backbone_arch = str(getattr(config, "fno_cnn_backbone_arch", "fno")).lower()
        if arch == "tfno_cnn":
            backbone_arch = "tfno"
        if backbone_arch not in {"fno", "tfno"}:
            raise ValueError(f"Unsupported fno_cnn_backbone_arch: {backbone_arch}")

        backbone_cls = FNO if backbone_arch == "fno" else TFNO
        ...
        backbone = backbone_cls(**_filter_model_kwargs(backbone_cls, backbone_kwargs))
        return FNOCNNModel(backbone, config)
...
def compute_spatial_gradient_loss(
    pred: torch.Tensor,
    target: torch.Tensor,
    ...
    """
    Penalize errors in local slopes. This is useful for sea-surface fields because
    a visually small height error around crests/troughs may still imply a large
    local phase/slope error.
    ...
    if loss_type == "l1":
        loss_x = (torch.abs(pred_dx - target_dx) * time_weights).mean()
        loss_y = (torch.abs(pred_dy - target_dy) * time_weights).mean()
    else:
        loss_x = (((pred_dx - target_dx) ** 2) * time_weights).mean()
        loss_y = (((pred_dy - target_dy) ** 2) * time_weights).mean()
...
def compute_rollout_loss(
    ...
    grad_loss_weight = float(getattr(config, "spatial_gradient_loss_weight", 0.0))
    grad_loss_type = str(getattr(config, "spatial_gradient_loss_type", "mse"))
    ...
    for i, (pred_i, target_i) in enumerate(zip(preds, targets)):
        ...
        data_loss_i = compute_weighted_mse(pred_i.float(), target_i.float(), time_weight_cache[take])
        loss_i = data_loss_i

        if use_grad_loss and grad_loss_weight > 0.0:
            grad_loss_i = compute_spatial_gradient_loss(
                pred_i.float(),
                target_i.float(),
                time_weight_cache[take],
                loss_type=grad_loss_type,
            )
            loss_i = loss_i + grad_loss_weight * grad_loss_i
            grad_values.append(float(grad_loss_i.detach().item()))

        total_loss = total_loss + segment_weights[i] * loss_i
        mse_values.append(float(data_loss_i.detach().item()))

    total_loss = total_loss / sum_weights
    return total_loss, {
        "loss": float(total_loss.detach().item()),
        "mse": float(np.mean(mse_values)) if mse_values else 0.0,
        "grad": float(np.mean(grad_values)) if grad_values else 0.0,
    }
...
def main() -> None:
    ...
    summary_row = {
        ...
        "fno_cnn_base_channels": int(getattr(config, "fno_cnn_base_channels", -1)),
        "fno_cnn_depth": int(getattr(config, "fno_cnn_depth", -1)),
        "fno_cnn_residual_scale": float(getattr(config, "fno_cnn_residual_scale", -1.0)),
        "fno_cnn_use_context": int(getattr(config, "fno_cnn_use_context", False)),
        "use_spatial_gradient_loss": int(getattr(config, "use_spatial_gradient_loss", False)),
