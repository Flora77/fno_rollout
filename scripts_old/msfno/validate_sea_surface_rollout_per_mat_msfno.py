...
from scripts.msfno.train_sea_surface_rollout_msfno import build_model
...
def sync_if_cuda(device: torch.device) -> None:
    """Synchronize CUDA so rollout timing reflects real GPU execution time."""
...
def evaluate_rollout(model: torch.nn.Module, loader: DataLoader, device: torch.device, config: SeaSurfaceRolloutConfig, data_mean: float, data_std: float, split: str) -> Dict[str, Any]:
    ...
    # only measure the trained model's prediction time from input_steps to rollout_steps.
    # This excludes DataLoader loading, host-to-device transfer, metric computation,
    # spectrum/statistics computation, and .mat/.csv saving.
    ...
    for batch in loader:
        ...
        rollout_infer_num_samples += int(bsz)
    ...
    result = {
        ...
        "rollout_infer_time_total_sec": np.array(rollout_infer_time_total_sec, dtype=np.float64),
        "rollout_infer_time_per_sample_sec": np.array(rollout_infer_time_per_sample_sec, dtype=np.float64),
        "rollout_infer_time_per_batch_sec": np.array(rollout_infer_time_per_batch_sec, dtype=np.float64),
        "rollout_infer_time_batch_list_sec": rollout_infer_time_batch_arr.reshape(-1, 1),
        "rollout_infer_num_samples": np.array(rollout_infer_num_samples, dtype=np.int32),
        "rollout_infer_num_batches": np.array(rollout_infer_num_batches, dtype=np.int32),
        "rollout_infer_steps": np.array(rollout_steps, dtype=np.int32),
...
def main(split: str = "val") -> None:
    ...
    print(
        ...
        f"total={float(result['rollout_infer_time_total_sec']):.6f}s | "
        f"per_sample={float(result['rollout_infer_time_per_sample_sec']):.6f}s | "
        f"per_batch={float(result['rollout_infer_time_per_batch_sec']):.6f}s | "
        f"num_samples={int(result['rollout_infer_num_samples'])} | "
        f"num_batches={int(result['rollout_infer_num_batches'])} | "
        f"rollout_steps={int(result['rollout_infer_steps'])}"
    )
...
if __name__ == "__main__":
    split_arg = sys.argv[1] if len(sys.argv) > 1 else "val"
