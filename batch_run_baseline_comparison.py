...
# -*- coding: utf-8 -*-
"""
Batch runner for baseline comparison experiments G1-G6.

Run this file from the NEURALOPERATOR project root, or place it in the project
root and run:

    python batch_run_baseline_comparison.py --checkpoint_dir ./checkpoints/general_comparison_best_fno --splits val test

It will sequentially run:
    G1 FNO     + no-curriculum rollout
    G2 MSFNO   + no-curriculum rollout
    G3 FNO     + curriculum rollout
    G4 MSFNO   + curriculum rollout
    G5 ConvLSTM + no-curriculum rollout
    G6 ConvLSTM + curriculum rollout

python batch_run_baseline_comparison.py --only G5 --skip_train --dry_run
python batch_run_baseline_comparison.py --dry_run     
python batch_run_baseline_comparison.py      

Notes
-----
1) The FNO/MSFNO training script already exposes CLI arguments for model_arch and
   rollout_train_steps, but the ConvLSTM training script may not expose
   rollout_train_steps. Therefore this runner temporarily patches the config file
   before each experiment and restores the original config files at the end.
2) Training/validation stdout is saved under:
       <checkpoint_dir>/batch_logs/
3) Existing train/val/test summary CSVs are not deleted. This runner appends to
   the normal summary files produced by your train/validate scripts and also
   writes a filtered combined CSV:
       <checkpoint_dir>/baseline_comparison_filtered_summary.csv
"""
...
class ExperimentSpec:
    gid: str
    title: str
    family: str                         # "convlstm" or "rno_msfno"
    model_arch: str                      # "convlstm", "fno", "msfno"
    experiment_name: str
...
def find_project_root(start: Path) -> Path:
    """Find the project root that contains config/ and scripts/."""
    start = start.resolve()
...
def as_python_literal(value: Any) -> str:
    """Return a compact Python literal for config-file replacement."""
...
def patch_dataclass_config_text(text: str, updates: Dict[str, Any], config_path: Path) -> str:
    """
    Replace dataclass default assignments, preserving type annotation and comments.

    Example line:
        rollout_train_steps: tuple = (30, 60, 120)  # comment
    becomes:
...
def build_experiments() -> List[ExperimentSpec]:
    conv_config = "config/sea_surface_rollout_config_convlstm.py"
    conv_train = "scripts/convlstm_log/train_sea_surface_rollout_convlstm.py"
    conv_val = "scripts/convlstm_log/validate_sea_surface_rollout_convlstm.py"

    rno_config = "config/sea_surface_rollout_config_rno_msfno.py"
    rno_train = "scripts/msfno_rno_log/train_sea_surface_rollout_rno_msfno.py"
    rno_val = "scripts/msfno_rno_log/validate_sea_surface_rollout_rno_msfno.py"

    return [

        ExperimentSpec(
            gid="G1",
            title="FNO + no curriculum rollout",
            family="rno_msfno",
            model_arch="fno",
            experiment_name="G2_fno_no_curriculum_rollout300",
            rollout_train_steps=NO_CURRICULUM_STEPS,
            rollout_curriculum_boundaries=NO_CURRICULUM_BOUNDARIES,
            config_relpath=rno_config,
            train_relpath=rno_train,
            validate_relpath=rno_val,
        ),
        ExperimentSpec(
            gid="G2",
            title="MSFNO + no curriculum rollout",
            family="rno_msfno",
            model_arch="msfno",
            experiment_name="G2_msfno_no_curriculum_rollout300",
            rollout_train_steps=NO_CURRICULUM_STEPS,
            rollout_curriculum_boundaries=NO_CURRICULUM_BOUNDARIES,
            config_relpath=rno_config,
            train_relpath=rno_train,
        ...
        ExperimentSpec(
            gid="G3",
            title="FNO + curriculum rollout",
            family="rno_msfno",
            model_arch="fno",
            experiment_name="G3_fno_curriculum_rollout300",
        ...
        ExperimentSpec(
            gid="G4",
            title="MSFNO + curriculum rollout",
            family="rno_msfno",
            model_arch="msfno",
            experiment_name="G4_msfno_curriculum_rollout300",
            rollout_train_steps=CURRICULUM_STEPS,
            rollout_curriculum_boundaries=CURRICULUM_BOUNDARIES,
            config_relpath=rno_config,
            train_relpath=rno_train,
        ...
        ExperimentSpec(
            gid="G5",
            title="ConvLSTM + no curriculum rollout",
            family="convlstm",
            model_arch="convlstm",
            experiment_name="G5_convlstm_no_curriculum_rollout300",
            rollout_train_steps=NO_CURRICULUM_STEPS,
            rollout_curriculum_boundaries=NO_CURRICULUM_BOUNDARIES,
            config_relpath=conv_config,
            train_relpath=conv_train,
            validate_relpath=conv_val,
        ),   
        ExperimentSpec(
            gid="G6",
            title="ConvLSTM + curriculum rollout",
            family="convlstm",
            model_arch="convlstm",
            experiment_name="G6_convlstm_curriculum_rollout300",
            rollout_train_steps=CURRICULUM_STEPS,
            rollout_curriculum_boundaries=CURRICULUM_BOUNDARIES,
            config_relpath=conv_config,
            train_relpath=conv_train,
...
def build_config_updates(exp: ExperimentSpec, checkpoint_dir: str, data_root: Optional[str]) -> Dict[str, Any]:
    updates: Dict[str, Any] = {
        ...
        "use_long_rollout_curriculum": exp.use_long_rollout_curriculum,
        "rollout_train_steps": exp.rollout_train_steps,
        "rollout_curriculum_boundaries": exp.rollout_curriculum_boundaries,
        "rollout_steps": 300,
        "output_steps": 30,
...
def build_train_command(
    ...
    cmd += ["--output_steps", "30", "--rollout_steps", "300"]

    # FNO/MSFNO training script accepts these extra CLI overrides.
    # ConvLSTM rollout_train_steps is controlled by config patching above.
    if exp.family == "rno_msfno":
    ...
    for summary_type, csv_path in summary_files:
        for row in read_csv_rows(csv_path):
            if row.get("experiment_name") in exp_names:
                row = dict(row)
                row.setdefault("summary_type", summary_type)
                row["summary_type"] = summary_type
                rows.append(row)
    out_path = checkpoint_dir / "baseline_comparison_filtered_summary.csv"
...
def write_manifest(checkpoint_dir: Path, experiments: Sequence[ExperimentSpec], args: argparse.Namespace) -> Path:
    ...
    out = checkpoint_dir / "baseline_comparison_manifest.json"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Batch-run G1-G6 comparison experiments.")
    parser.add_argument("--checkpoint_dir", type=str, default="./checkpoints/general_comparison_best_fno",
    ...
    parser.add_argument("--only", nargs="*", default=None,
                        help="Optional experiment ids to run, e.g. --only G1 G4.")
...
def main() -> None:
    ...
    # Validate required scripts/configs early.
    required_relpaths = sorted({e.config_relpath for e in experiments} | {e.train_relpath for e in experiments} | {e.validate_relpath for e in experiments})
    for rel in required_relpaths:
        path = root / rel
        if not path.exists():
            raise FileNotFoundError(f"Required file not found: {path}")

    # Read originals once and restore them in finally.
    ...
    try:
        for exp in experiments:
            ...
            # Patch the corresponding config file for this experiment.
        ...
        # Restore user config files, even if an experiment fails.
