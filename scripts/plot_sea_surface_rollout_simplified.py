...
def plot_point_traces(data: Dict, save_dir: str, config: SeaSurfaceRolloutConfig, metrics_lines) -> None:
    ...
    num_samples = min(int(config.plot_num_samples), n_samples)
    ...
    metrics_lines.append("Point order follows saved point_true_all / point_pred_all / point_names in updated2 MAT.")

    for s in range(num_samples):
        sample_id = int(data["saved_sample_ids"][s]) if s < len(data["saved_sample_ids"]) else s
        ...
        metrics_lines.append(f"  sample_index={s}, saved_sample_id={sample_id}")
        ...
        fig.suptitle(
            f"Point traces of sample {s} (saved_sample_id={sample_id})",
            fontsize=12,
        )
...
def plot_sample_fields(data: Dict, save_dir: str, config: SeaSurfaceRolloutConfig, metrics_lines) -> None:
    ...
    num_samples = min(int(config.plot_num_samples), x_plot.shape[0])
    ...
    for s in range(num_samples):
        ...
        metrics_lines.append("=" * 80)
        metrics_lines.append(f"[Sample Fields] sample_index={s}, saved_sample_id={sample_id}")
