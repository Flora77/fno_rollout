...
try:
    from scripts.convlstm_log.train_sea_surface_rollout_convlstm import build_model
except ImportError:
    # Useful when this validation file is placed next to the training script or renamed for experiments.
    from train_sea_surface_rollout_convlstm import build_model
...
def update_paper_metric_accumulator(acc: Dict[str, float], y_true_phy: np.ndarray, y_pred_phy: np.ndarray) -> None:
    """Update NRMSE/SSP/Corr accumulator using denormalized physical sea-surface elevations."""
...
def evaluate_rollout(model: torch.nn.Module, loader: DataLoader, device: torch.device, config: SeaSurfaceRolloutConfig, data_mean: float, data_std: float, split: str) -> Dict[str, Any]:
    ...
    # Paper metrics: compute per source .mat first, then average across files.
    ...
    # Timing: only model rollout inference is counted.
    ...
    rollout_infer_time_per_batch_sec = float(np.mean(rollout_infer_time_batch_arr)) if rollout_infer_time_batch_arr.size else float("nan")
...
def parse_args():
    parser = argparse.ArgumentParser(description="Validate/test ConvLSTM rollout model with FNO-UNet-compatible metrics")
...
def main(
    ...
    result.update({
        ...
        "convlstm_input_channels": np.array(int(config.convlstm_input_channels), dtype=np.int32),
        "convlstm_hidden_channels": np.array(int(config.convlstm_hidden_channels), dtype=np.int32),
        "convlstm_num_layers": np.array(int(config.convlstm_num_layers), dtype=np.int32),
        "convlstm_kernel_size": np.array(int(config.convlstm_kernel_size), dtype=np.int32),
        "convlstm_output_kernel_size": np.array(int(config.convlstm_output_kernel_size), dtype=np.int32),
    })
    ...
    summary_row = {
        ...
        "convlstm_input_channels": int(config.convlstm_input_channels),
        "convlstm_hidden_channels": int(config.convlstm_hidden_channels),
        "convlstm_num_layers": int(config.convlstm_num_layers),
        "convlstm_kernel_size": int(config.convlstm_kernel_size),
        "convlstm_output_kernel_size": int(config.convlstm_output_kernel_size),
        "convlstm_decoder_input": str(config.convlstm_decoder_input),
