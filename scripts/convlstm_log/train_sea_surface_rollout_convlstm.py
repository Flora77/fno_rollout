...
def _find_project_root(start_file: Path) -> Path:
    ...
    for parent in [start.parent, *start.parents]:
        if (parent / "neuralop").exists() and (parent / "config").exists():
            return parent
    # Fallback compatible with scripts/<subdir>/file.py inside project root.
...
def compute_spatial_gradient_mse(pred: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
    """MSE of spatial slopes. Matches the FNO-UNet metrics training script."""
...
def apply_cli_overrides(config: SeaSurfaceRolloutConfig) -> SeaSurfaceRolloutConfig:
    """Small CLI helper for ConvLSTM ablations and fair comparison runs."""
    parser = argparse.ArgumentParser(description="Train ConvLSTM rollout model with FNO-UNet-compatible metrics")
    ...
    if args.hidden_channels is not None:
        config.convlstm_hidden_channels = int(args.hidden_channels)
    if args.num_layers is not None:
        config.convlstm_num_layers = int(args.num_layers)
    if args.kernel_size is not None:
        config.convlstm_kernel_size = int(args.kernel_size)
...
def main() -> None:
    ...
    # Curriculum stages need different y lengths, so each active rollout gets one loader.
