...
from config.sea_surface_rollout_config_rno_msfno import SeaSurfaceRolloutConfig
...
def _parse_tuple_arg(value: Any, item_type=float) -> Tuple:
    """Parse comma-separated CLI values such as '0.5,1,2,4'."""
    if value is None:
        return tuple()
    if isinstance(value, str):
...
def apply_cli_overrides(config: SeaSurfaceRolloutConfig) -> SeaSurfaceRolloutConfig:
    """CLI helper so one script can train RNO/FNO and MSFNO under the same protocol."""
    parser = argparse.ArgumentParser(description="Train FNO/RNO or MSFNO rollout model with comparable metrics")
    parser.add_argument("--model_arch", type=str, default=None,
                        choices=["fno", "tfno", "msfno", "msfno2d", "mscale_fno", "mscalefno"])
    ...
    parser.add_argument("--input_steps", type=int, default=None)
    parser.add_argument("--output_steps", type=int, default=None)
    parser.add_argument("--rollout_steps", type=int, default=None)
    parser.add_argument("--rollout_train_steps", type=str, default=None,
                        help="Comma-separated rollout lengths, e.g. 30,60,120,180,240,300")
    parser.add_argument("--rollout_curriculum_boundaries", type=str, default=None,
                        help="Comma-separated curriculum boundaries, e.g. 0,0.1,0.2,0.3,0.45,0.6")
    parser.add_argument("--use_long_rollout_curriculum", type=int, default=None)
    ...
    parser.add_argument("--use_spatial_gradient_loss", type=int, default=None)
    parser.add_argument("--spatial_gradient_loss_weight", type=float, default=None)
    parser.add_argument("--msfno_scales", type=str, default=None,
                        help="Comma-separated MSFNO branch scales, e.g. 0.5,1,2,4")
    parser.add_argument("--msfno_branch_width_factor", type=float, default=None)
    parser.add_argument("--msfno_branch_hidden_channels", type=int, default=None)
    parser.add_argument("--msfno_branch_lifting_channels", type=int, default=None)
    parser.add_argument("--msfno_branch_projection_channels", type=int, default=None)
    parser.add_argument("--msfno_fusion", type=str, default=None, choices=["conv", "weighted_sum", "mean"])
    parser.add_argument("--msfno_conv_hidden_channels", type=str, default=None,
                        help="Comma-separated 3D conv fusion hidden channels, e.g. 8,8 or 32,64,32")
    args = parser.parse_args()

    if args.model_arch is not None:
        config.model_arch = str(args.model_arch)
    ...
    if args.val_batch_size is not None:
        config.val_batch_size = int(args.val_batch_size)
    if args.test_batch_size is not None:
        config.test_batch_size = int(args.test_batch_size)
    if args.learning_rate is not None:
        config.learning_rate = float(args.learning_rate)
    if args.input_steps is not None:
        config.input_steps = int(args.input_steps)
    if args.output_steps is not None:
        config.output_steps = int(args.output_steps)
    if args.rollout_steps is not None:
        config.rollout_steps = int(args.rollout_steps)
    if args.rollout_train_steps is not None:
        config.rollout_train_steps = tuple(int(v) for v in _parse_tuple_arg(args.rollout_train_steps, int))
    if args.rollout_curriculum_boundaries is not None:
        config.rollout_curriculum_boundaries = tuple(float(v) for v in _parse_tuple_arg(args.rollout_curriculum_boundaries, float))
    if args.use_long_rollout_curriculum is not None:
        config.use_long_rollout_curriculum = bool(args.use_long_rollout_curriculum)
    ...
    if args.use_spatial_gradient_loss is not None:
        config.use_spatial_gradient_loss = bool(args.use_spatial_gradient_loss)
    if args.spatial_gradient_loss_weight is not None:
        config.spatial_gradient_loss_weight = float(args.spatial_gradient_loss_weight)
    if args.msfno_scales is not None:
        config.msfno_scales = tuple(float(v) for v in _parse_tuple_arg(args.msfno_scales, float))
    if args.msfno_branch_width_factor is not None:
        config.msfno_branch_width_factor = float(args.msfno_branch_width_factor)
    if args.msfno_branch_hidden_channels is not None:
        config.msfno_branch_hidden_channels = int(args.msfno_branch_hidden_channels)
    if args.msfno_branch_lifting_channels is not None:
        config.msfno_branch_lifting_channels = int(args.msfno_branch_lifting_channels)
    if args.msfno_branch_projection_channels is not None:
        config.msfno_branch_projection_channels = int(args.msfno_branch_projection_channels)
    if args.msfno_fusion is not None:
        config.msfno_fusion = str(args.msfno_fusion)
    if args.msfno_conv_hidden_channels is not None:
        config.msfno_conv_hidden_channels = tuple(int(v) for v in _parse_tuple_arg(args.msfno_conv_hidden_channels, int))
...
def main() -> None:
    ...
    summary_row = {
        ...
        "rollout_steps": int(config.rollout_steps),
        "msfno_scales": str(tuple(getattr(config, "msfno_scales", ()))),
        "msfno_branch_width_factor": float(getattr(config, "msfno_branch_width_factor", 0.0)),
        "msfno_branch_hidden_channels": int(getattr(config, "msfno_branch_hidden_channels", 0)),
        "msfno_branch_lifting_channels": int(getattr(config, "msfno_branch_lifting_channels", 0)),
        "msfno_branch_projection_channels": int(getattr(config, "msfno_branch_projection_channels", 0)),
        "msfno_fusion": str(getattr(config, "msfno_fusion", "")),
        "msfno_conv_hidden_channels": str(tuple(getattr(config, "msfno_conv_hidden_channels", ()))),
        "msfno_conv_kernel_size": str(tuple(getattr(config, "msfno_conv_kernel_size", ()))),
        "msfno_conv_norm": str(getattr(config, "msfno_conv_norm", "")),
        "msfno_conv_activation": str(getattr(config, "msfno_conv_activation", "")),
