...
# Reuse the exact model builder from the unified training script so FNO/RNO and MSFNO
# checkpoints are reconstructed consistently during validation.
try:
    from scripts.msfno_rno_log.train_sea_surface_rollout_rno_msfno import build_model
except ImportError:
    try:
        from scripts.msfno_rno_log.train_sea_surface_rollout_rno_msfno import build_model
    except ImportError:
        try:
            ...
            raise ImportError(
                "Could not import build_model. Put train_sea_surface_rollout_rno_msfno_metrics.py "
                "in the same directory as this validation script or under scripts/msfno/."
            ) from e
...
# build_model is imported from the unified training script.
# This avoids maintaining a second copy of the FNO/MSFNO architecture here.
...
def evaluate_rollout(model: torch.nn.Module, loader: DataLoader, device: torch.device, config: SeaSurfaceRolloutConfig, data_mean: float, data_std: float, split: str) -> Dict[str, Any]:
    ...
    for file_id in per_file_ids.tolist():
        metrics_i = finalize_paper_metric_accumulator(per_file_metric_accs[int(file_id)])
        ...
        per_file_num_samples.append(per_file_sample_counts.get(int(file_id), 0))
...
def main(
    ...
    result.update({
        ...
        "input_steps": np.array(int(config.input_steps), dtype=np.int32),
        "output_steps": np.array(int(config.output_steps), dtype=np.int32),
        "rollout_steps": np.array(int(config.rollout_steps), dtype=np.int32),
        "msfno_scales": np.asarray(str(tuple(getattr(config, "msfno_scales", ()))), dtype=object),
        "msfno_branch_width_factor": np.array(float(getattr(config, "msfno_branch_width_factor", 0.0)), dtype=np.float64),
        "msfno_fusion": np.asarray(str(getattr(config, "msfno_fusion", "")), dtype=object),
        "msfno_conv_hidden_channels": np.asarray(str(tuple(getattr(config, "msfno_conv_hidden_channels", ()))), dtype=object),
    })
...
if __name__ == "__main__":
    # Usage examples:
    #   python validate_sea_surface_rollout_per_mat_rno_msfno_metrics.py val --experiment_name fno_single_unet_gated
    #   python validate_sea_surface_rollout_per_mat_rno_msfno_metrics.py test --experiment_name fno_single_aunet_gated
    args = parse_args()
    main(
        split=args.split,
