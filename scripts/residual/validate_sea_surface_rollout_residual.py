...
def build_model(config: SeaSurfaceRolloutConfig) -> torch.nn.Module:
    ...
    model_cls = FNO if arch == "fno" else TFNO if arch == "tfno" else None
...
def evaluate_rollout(model: torch.nn.Module, loader: DataLoader, device: torch.device, config: SeaSurfaceRolloutConfig, data_mean: float, data_std: float, split: str) -> Dict[str, Any]:
    ...
    result = {
        ...
        "use_residual_prediction": np.array(int(getattr(config, "use_residual_prediction", False)), dtype=np.int32),
        "residual_base_mode": np.asarray(str(getattr(config, "residual_base_mode", "none")), dtype=object),
        "residual_linear_lag": np.array(int(getattr(config, "residual_linear_lag", 0)), dtype=np.int32),
    }
    return result
