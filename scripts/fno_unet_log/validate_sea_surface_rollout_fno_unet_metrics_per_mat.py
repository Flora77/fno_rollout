...
def _pathlike_to_name(value: Any) -> str:
    """Return a .mat filename from a path-like/list/dict/object item, or empty string."""
    if value is None:
        return ""

    # Common case: pathlib.Path or string.
    ...
    # Some datasets store one file record as a dict.
    if isinstance(value, dict):
        for key in (
            "file_name", "filename", "name", "mat_file_name",
            "file_path", "filepath", "path", "mat_path", "mat_file",
        ):
            if key in value:
                name = _pathlike_to_name(value.get(key))
                if name:
                    return name
        return ""

    # Some datasets store one file record as a tuple/list, e.g. (path, metadata).
    if isinstance(value, (list, tuple)):
        for item in value:
            name = _pathlike_to_name(item)
            if name:
                return name
        return ""

    # Fallback for simple custom objects with path/name attributes.
    for attr in (
        "file_name", "filename", "name", "mat_file_name",
        "file_path", "filepath", "path", "mat_path", "mat_file",
    ):
        if hasattr(value, attr):
            name = _pathlike_to_name(getattr(value, attr))
            if name:
                return name
...
def _file_name_from_candidate(candidate: Any, file_id: int) -> str:
    """Try to get the filename for file_id from a dataset attribute candidate."""
    if candidate is None:
        return ""

    # Mapping: {file_id: path/name} or {"0": path/name}.
    if isinstance(candidate, dict):
        for key in (file_id, str(file_id)):
            if key in candidate:
                name = _pathlike_to_name(candidate[key])
                if name:
                    return name
        return ""

    # Sequence or numpy array of paths/records.
    try:
        if isinstance(candidate, np.ndarray):
            seq = candidate.tolist()
        else:
            seq = candidate
        if hasattr(seq, "__len__") and 0 <= file_id < len(seq):
            name = _pathlike_to_name(seq[file_id])
            if name:
                return name
    except Exception:
        return ""
...
def build_dataset_file_name_resolver(eval_dataset: Any, eval_dir: str):
    """
    Build a robust resolver: file_id -> original .mat filename.

    It first checks common SeaSurfaceSimpleDataset attributes. If these are absent,
    it scans all public dataset attributes for path-like lists/dicts. As a final
    fallback, it uses the sorted *.mat files in the evaluated split directory,
    which should match the dataset order for SeaSurfaceSimpleDataset.
    """
    candidates: List[Any] = []

    # Explicit likely attribute names from different dataset versions.
    explicit_attrs = (
        "mat_files", "mat_file_paths", "mat_paths", "mat_filenames", "mat_file_names",
        "file_paths", "file_names", "filenames", "files", "paths", "data_files",
        "all_files", "all_file_paths", "file_list", "mat_file_list",
    )
    for attr_name in explicit_attrs:
        if hasattr(eval_dataset, attr_name):
            candidates.append(getattr(eval_dataset, attr_name))

    # Search public attributes defensively. This helps if the dataset uses a new
    # name such as self.file_infos or self.samples_by_file.
    try:
        for attr_name, value in vars(eval_dataset).items():
            if attr_name.startswith("_"):
                continue
            if value is None or value in candidates:
                continue
            if isinstance(value, dict):
                # Keep dicts that look like file-id -> path/name records.
                sample_values = list(value.values())[:5]
                if any(_pathlike_to_name(v) for v in sample_values):
                    candidates.append(value)
            elif isinstance(value, (list, tuple, np.ndarray)):
                try:
                    sample_values = list(value[:5]) if not isinstance(value, np.ndarray) else value.reshape(-1)[:5].tolist()
                except Exception:
                    sample_values = []
                if any(_pathlike_to_name(v) for v in sample_values):
                    candidates.append(value)
    ...
    # Final fallback: sorted .mat files in data_root/val or data_root/test.
    fallback_files: List[Path] = []
    try:
        fallback_files = sorted(Path(eval_dir).glob("*.mat"))
        if fallback_files:
            candidates.append(fallback_files)
    except Exception:
        fallback_files = []

    def resolve(file_id: int) -> str:
        file_id = int(file_id)
        for candidate in candidates:
            name = _file_name_from_candidate(candidate, file_id)
            if name:
                return name
        return ""
...
def evaluate_rollout(model: torch.nn.Module, loader: DataLoader, device: torch.device, config: SeaSurfaceRolloutConfig, data_mean: float, data_std: float, split: str) -> Dict[str, Any]:
    ...
    # Map dataset file_id back to the original .mat filename.
    # This is used by the per-mat CSV so each metric row can be traced to the
    # source sea-state file. The resolver is robust to different dataset versions
    # and falls back to sorted *.mat files in data_root/split.
    eval_dir_for_names = os.path.join(config.data_root, split)
    resolve_mat_file_name = build_dataset_file_name_resolver(eval_dataset, eval_dir_for_names)
    ...
    for file_id in per_file_ids.tolist():
        ...
        file_name = resolve_mat_file_name(file_id_int)
